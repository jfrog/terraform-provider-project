...
docker run -i -t -d --rm -v "${SCRIPT_DIR}/artifactory.lic:/artifactory_extra_conf/artifactory.lic:ro" \
  -p 8081:8081 -p 8082:8082  releases-docker.jfrog.io/jfrog/projects-pro:7.24.3
...
curl -u admin:password  --output /dev/null --silent --fail localhost:8081/projects/api/system/decrypt -X POST
