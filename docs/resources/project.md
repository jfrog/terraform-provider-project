...
- **repos** (Set of String) List of existing repo keys to be assigned to the project. By default, you can set a maximum of 100 repo keys. (see [below for limitations on the maximum number of repos](#repoNumLimitations)). You can override the system's default max cap by setting an environment value `REPO_LIMIT_OVERRIDE` to `true`. This setting will remove the default restriction of maximum allowable elements in the `repos` attribute. The default value of `REPO_LIMIT_OVERRIDE` is `false`. E.g. To override the default maximum allowable elements of `repos` attribute use `export REPO_LIMIT_OVERRIDE=true`.
