...
- **repos** (Set of String) List of existing repo keys to be assigned to the project. You can set a maximum 100 repo keys. (se [below for limitations on the maximum number of repos](#repoNumLimitations)). You can override the default value by setting an environment value `REPO_LIMIT_OVERRIDE`. E.g. To override the default maximum allowable repos from 100 to 200 use `export REPO_LIMIT_OVERRIDE=200`.
