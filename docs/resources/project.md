...
~> This resource was previously named `project`. The non-namespaced `project` name is **deprecated** and will be removed in the next major version. Existing configurations keep working and emit a deprecation warning; you can migrate without destroying or recreating your projects by adding a `moved` block. See the [Migrating from the `project` resource](https://registry.terraform.io/providers/jfrog/project/latest/docs/guides/migrating_to_project_project) guide.
